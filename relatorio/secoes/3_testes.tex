...
Para fazer os testes, considerei alguns problemas-modelo que são coreografias (trajetórias periódicas), que permitem estudar a estabilidade numérica de métodos de integração. As trajetórias dos problema-modelo periódicos estão disponibilizadas abaixo, e os valores iniciais podem ser encontrados no repositório \href{https://github.com/Potalej/MPT-2026/}{https://github.com/Potalej/MPT-2026/}.
...
\begin{probmodelo}[Coreografia-Doicu 5 corpos \cite{Doicu2022}]\label{pm:doicu5}
...
\begin{probmodelo}[Coreografia-Doicu 12 corpos \cite{Doicu2022}]\label{pm:doicu12}
...
\end{equation}
Além disso, consideramos $\delta t = \alpha \Delta t$, então
\begin{equation}
    S(k) \approx \dfrac{1}{(1 + k) \alpha \dfrac{R_C}{R_{\mathcal F}} + \dfrac{k}{N_P}}.
\end{equation}

Nos experimentos feitos na seção de testes, tomaremos $R_C=1$ e $R_C=2$, e $R_{\mathcal F}=2$ e $R_{\mathcal F}=4$. Além disso, $\alpha=4/32$ ou $\alpha=1/32$. Conforme a figura \ref{fig:funcao_S}, isso nos deixa entre 7 e 13 iterações para haver vantagem em termos de tempo no Parareal.

\begin{figure}[H]
    \centering
    \includegraphics[width=.5\linewidth]{img/funcao_S.png}
    \caption{Função $S(k)$ aproximada para $N_P=15$.}
    \label{fig:funcao_S}
\end{figure}

Sobre a convergência, é preciso escolher intervalos suficientemente pequenos para que movimentos oscilatórios não sejam ``visíveis'' ao método, no sentido de fazer crescer sua instabilidade; noutro caso, o método não deve convergir, ao menos não em uma quantidade relativamente pequena de iterações (até 30). Além disso, descontinuidades podem afetar a comunicação entre os propagadores fino e grosseiro, então também se pode esperar que o Parareal não funcione tão bem quando se adiciona choques elásticos ao problema.
...
O problema de 5 corpos (p.m. \ref{pm:doicu5}) já não converge tão rapidamente quanto à lemniscata. Por isso, também serão utilizados métodos de segunda ordem como métodos grosseiros.
